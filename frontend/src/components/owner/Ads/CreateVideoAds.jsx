...
import { ArrowLeft, Video, X, Check } from 'lucide-react';
import { useNavigate } from 'react-router-dom';

const CreateVideoAds = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!video || !budget || !agreeTerms || !title || !description) {
        ...
        try {
            setLoading(true);
            setMessage('');
            await axios.post('https://offerwhisky.onrender.com/api/ad/create-video-ads', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
            });

            setMessage('Video Ad created successfully!');
            setVideo(null);
            setPreview('');
            setBudget(500);
            setManualReach('');
            ...
            setMessage('Failed to create video ad');
            console.log(err);
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="min-h-screen bg-white pb-36">
            <div className="flex items-center p-4 border-b">
                ...
                <h1 className="text-xl font-bold">Create Video Ad</h1>
            </div>
            ...
            <div className="p-4">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h2 className="font-semibold">Video Ad</h2>
                        <p className="text-sm text-gray-600">Upload promotional video with description</p>
                    </div>
                </div>
            </div>
                ...
                <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center">
                    {!preview ? (
                        <div>
                            ...
                            <p className="text-sm text-gray-600 mb-2">MP4, MOV or AVI up to 50MB</p>
                            <label className="cursor-pointer">
                                <span className="px-4 py-2 bg-blue-600 text-white rounded-lg">Select Video</span>
            ...
            <div className="m-4 space-y-2">
                ...
                <div className="p-6 bg-[#f0f7ff] rounded-xl border">
                    <div className="flex justify-between items-center">
                        <div>
                            <h3 className="text-lg font-semibold text-gray-700">Est. Reach</h3>
                            <p className="text-sm text-gray-500 mt-1">Potential viewers</p>
                        </div>
                        <div className="text-right flex items-baseline">
                            <p className="text-[2rem] font-bold text-[#2463eb]">
                ...
                <div className="mt-4 p-4 bg-[#f0f7ff] rounded-xl border">
                    <div className="flex justify-between items-center">
                        <div>
                            <h3 className="text-lg font-semibold text-gray-700">Total Budget</h3>
                        </div>
                        <div className="text-right">
                            <p className="text-[2rem] font-bold text-[#2463eb]">₹{budget}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-5 p-4 bg-[#fffbea] border border-[#ffeeba] rounded-xl m-4">
                <div className="flex items-start" onClick={() => setAgreeTerms(!agreeTerms)}>
                    <div className={`flex-shrink-0 w-6 h-6 rounded mr-3 cursor-pointer mt-0.5 ${agreeTerms ? 'bg-[#2463eb]' : 'border-2 border-gray-300'}`}>
                    ...
                    <div className="flex-1">
                        <label htmlFor="terms" className="text-gray-700 text-sm leading-relaxed cursor-pointer block">
                            I agree to the <span className="text-[#2463eb] font-medium">Sponsored Ads Terms</span> and understand that charges will apply to my account.
                        </label>
                        <input
                            type="checkbox"
                            id="terms"
                ...
                <div className="border-b border-gray-100 py-3 px-4 bg-gray-50">
                    <div className="flex justify-between items-center max-w-lg mx-auto">
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="text-[#2463eb] font-bold text-xl">₹{budget}</div>
                        </div>
                    </div>
                </div>
                <div className="p-4 max-w-lg mx-auto">
                    <div className="flex gap-3">
                        ...
                        <button
                            ...
                            className={`flex-1 py-3.5 px-6 rounded-xl text-white font-medium flex items-center justify-center ${agreeTerms ? 'bg-blue-700 hover:bg-blue-800' : 'bg-gray-400 cursor-not-allowed'}`}
                        >
